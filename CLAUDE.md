...
- `$HOME/debian-setup`, the `DEBIAN_SETUP_*` knobs and the sudoers drop-in are named for the generic half even though the VM this provisions is usually the Claude one. The exception is the signing key at `~/.ssh/claude`, which is Claude-side and whose path `claude-dotfiles`' `.gitconfig` points `user.signingkey` at — moving it here means moving it there too.
